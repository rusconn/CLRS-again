...
> 以下の属性で表現される、添字 $6$ を根とする根付き２分木を描け。
...
![10.4-1の図](../diagram/10.4-1.png)

## 10.4-2

> 節点数 $n$ の２分木を入力とし、 $O(n)$ 時間でこの木に属するすべての節点のキーを印刷する再帰的手続きを記述せよ。

```pseudo
PRINT-KEYS(T):
  PRINT-KEYS-RECUR(T.root)
```

```pseudo
PRINT-KEYS-RECUR(root):
  if root ≠ NIL
    PRINT(root.key)
    PRINT-KEYS-RECUR(root.left)
    PRINT-KEYS-RECUR(root.right)
```

## 10.4-3

> 節点数 $n$ の２分木を入力とし、 $O(n)$ 時間でこの木に属するすべての節点のキーを印刷する非再帰的手続きを記述せよ。補助データ構造としてスタックを使え。
...
> 節点数 $n$ の任意の木を入力とし、 $O(n)$ 時間でこの木に属するすべての節点のキーを印刷する手続きを記述せよ。ただし、木は左-子、右-兄弟表現を用いて格納されている。

[練習問題 10.4-2](#104-2), [練習問題 10.4-3](#104-3) の手続きで対応できる。  
属性名が異なるなら、該当箇所を置き換えるだけ。

## 10.4-5 ★

> 節点数 $n$ の２分木を入力とし、 $O(n)$ 時間でこの木に属するすべての節点のキーを印刷する非再帰的手続きを記述せよ。ただし、木の表現以外には $O(1)$ の領域しか利用してはならず、木は一時的にせよ変形してはならない。

？

## 10.4-6 ★

> 左-子、右-兄弟表現では各節点について３つのポインタ、 $left\text{-}child$ , $right\text{-}child$ , $parent$ を用いる。この表現を用いると、どの節点からでも、その親に定数時間で到達、識別でき、そのすべての子に子の数の線形時間で到達、識別できる。各節点に対して２つのポインタと１つのブール値を使用して同じ操作を同じ時間計算量で実現する方法を示せ。
