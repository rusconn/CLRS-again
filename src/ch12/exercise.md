...
> 手続き $\text{TREE-INSERT}$ の再帰版を示せ。

```pseudo
TREE-INSERT(T, z):
  TREE-INSERT-RECUR(T, z, NIL, T.root)
```

```pseudo
TREE-INSERT-RECUR(T, z, y, x):
  if x == NIL
    z.p = y
    if y == NIL
      T.root = z
    ...
    if z.key < x.key
      TREE-INSERT-RECUR(T, z, x, x.left)
    else
      TREE-INSERT-RECUR(T, z, x, x.right)
```

## 12.3-2

> 異なる値を繰り返し挿入することで構成された任意の $2$ 分探索木を考える。ある値を探索するときに検査する節点数は、その値を挿入したときに検査した節点数に $1$ を加えたものであることを証明せよ。

スキップ

## 12.3-3

> 与えられた $n$ 個の数を以下の要領でソートできる。まず、 $n$ 個の数を（ $1$ つずつ繰り返し $\text{TREE-INSERT}$ を用いて挿入して）格納する $2$ 分探索木を構成する。次に、木の中間順巡回によってそれらの数を印刷する。このソーティングアルゴリズムの最良実行時間と最悪実行時間を求めよ。

スキップ

## 12.3-4

> 削除操作は、 $x,y$ の順に削除しても、逆に $y,x$ の順に削除しても同じ $2$ 分探索木が残るという意味で「可換」か？証明するか、あるいは反例を与えよ。
